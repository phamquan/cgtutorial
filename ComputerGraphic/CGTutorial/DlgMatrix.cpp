...
void CDlgMatrix::ShowMatrix(CDC* cdc, CString m[16], int top, int left)
{
	cdc->MoveTo(left+5,top-5);
	cdc->LineTo(left-5,top-5);
	cdc->LineTo(left-5,top+80);
	...
	for(int i=0; i<4; i++) {
		for(int j=0; j<4; j++) {
			cdc->TextOutW(left+j*50,top+i*20,m[i+j*4]);
		}
	}
	left += 185;
	cdc->MoveTo(left-5,top-5);
...
void CDlgMatrix::ShowPoint(CDC* cdc, CHomoPoint point, int top, int left)
{
	char buff[128];
	cdc->MoveTo(left+5,top-5);
	cdc->LineTo(left-5,top-5);
